...
Output goes to `a11y-audits/example.gov/checkout/`.

## Step 2 — Name it and choose where it lands

```bash
node bin/auria.mjs https://example.gov/checkout --name checkout-v2 --out ./audits
```
...
{
  "pages": [{
    "url": "https://example.gov/checkout",
    "viewports": [
...
```bash
node bin/auria.mjs https://example.gov/checkout --color-scheme dark --reduced-motion
```
...
```bash
node bin/auria.mjs https://example.gov/checkout --tabs 40
```
...
node bin/auria.mjs https://example.gov/checkout --no-video --color-scheme dark

# Full run with markdown report and annotated screenshots
node bin/auria.mjs https://example.gov/checkout --md --screenshots
```
