...
**Verified end-to-end on Windows:** analysis + all report formats, `--crawl`, System.Speech video (175s mp4), and **neural Piper video (191s mp4, ffmpeg-valid)** with a real `en_US-amy-medium` voice. Docs freshened post-port (README status, CHANGELOG, cli.md env vars).

**NVDA:** `npx @guidepup/setup` run; NVDA now **starts** under `--nvda` (no "unavailable" error), confirming the guidepup environment is set up (it manages its own NVDA — the system `%APPDATA%\nvda\addons` folder is expected to stay empty). Full phrase capture requires an **interactive foreground desktop** (NVDA reads the focused window); from an automated/background shell the focus guard fires (`captured no tab stops — lost OS focus`) as designed. Real captures need a hands-off interactive run — see [guides/nvda-mode.md](guides/nvda-mode.md). Still unverified elsewhere: espeak-ng video on Linux.
